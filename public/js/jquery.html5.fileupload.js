...
(function($) {
	...
	var handleFile = function (settings, file) {
		...
		if (settings.resizeImage && !isImageSupported && settings.allowUploadOriginalImage) {
			console.log('WARN: Fall back to upload original un-resized image.');
		...
		if (!settings.resizeImage && canReadFormData) {
			console.log('INFO: Bypass file reading, insert file object into FormData object directly.');
			handleForm(settings, 'file', file, info);
		} else if (window.FileReader) {
			console.log('INFO: Using FileReader to do asynchronously file reading.');
			...
			reader.onerror = function (ev) {
				if (ev.target.error) {
					switch (ev.target.error) {
						...
						break;
						case 18:
						console.log('ERROR: File cannot be access due to security constrant.');
			...
			if (!settings.resizeImage) {
				if (canSendBinaryString) {
					...
					console.log('ERROR: No available method to extract file; allowDataInBase64 not set.');
			...
			console.log('WARN: FileReader does not exist, UI will be blocked when reading big file.');
			if (!settings.resizeImage) {
				try {
			...
			console.log('ERROR: No available method to extract file; this browser is not supported.');
	...
	var handleImage = function (settings, dataurl, info) {
		...
		img.onerror = function () {
			console.log('ERROR: <img> failed to load, file is not a supported image format.');
		...
		img.onload = function () {
			...
			if (!settings.forceResize && img.width === d.w && img.height === d.h) {
				console.log('INFO: Image demension is the same, send the original file.');
				if (canResizeImageToBinaryString) {
					...
					console.log('ERROR: No available method to send the original file; allowDataInBase64 not set.');
					settings.fileError.call(this, info, 'NO_BIN_SUPPORT_AND_BASE64_NOT_SET', 'No available method to extract file; allowDataInBase64 not set.');
				}
				return;
			}
			...
			if (canResizeImageToFile && canSendFormData) {
				...
				console.log('ERROR: No available method to extract image; allowDataInBase64 not set.');
				settings.fileError.call(this, info, 'NO_BIN_SUPPORT_AND_BASE64_NOT_SET', 'No available method to extract file; allowDataInBase64 not set.');
			}
		}
		img.src = dataurl;
	...
	var handleForm = function (settings, type, data, info) {
		if (canSendFormData && type === 'file') {
			...
			console.log('INFO: Using FormData to construct form.');
			...
			console.log('INFO: Concat our own multipart/form-data data string.');
						
			// A placeholder MIME type
			if (!info.type) info.type = 'application/octet-stream';

			if (/[^\x20-\x7E]/.test(info.name)) {
				console.log('INFO: Filename contains non-ASCII code, do UTF8-binary string conversion.');
			...
			if (window.XMLHttpRequest.prototype.sendAsBinary) {
				...
				console.log('INFO: Pass binary string to xhr.');
				settings.data = formdata;
			} else {
				// make a blob
				console.log('INFO: Convert binary string into Blob.');
			...
			console.log('INFO: Concat our own multipart/form-data data string; send the file in base64 because binary xhr is not supported.');
			...
			console.log('ERROR: Data is not given in processable form.');
	...
	var xhrupload = function (settings) {
		...
		if (typeof settings.data === 'string' && canSendBinaryString) {
			console.log('INFO: Using xhr.sendAsBinary.');
			...
			if ($(el).is('input[type=file]')) {
				...
				$(el).bind(
					...
					function () {
						...
						if (this.form !== null && this.form.length === 1) {
							this.form.reset();
						} else {
